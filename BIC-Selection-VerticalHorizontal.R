    ...
    for (j in 1:N2){
      for (l in 1:N3){
        ...
        if (i>1){
          ...
          res.thetas=ADMM_VH(Xstd, M, K, p,pcore, L, rho,lmbda_grid,maxiter1,maxiter2,maxiter3, tol1,tol2,tol3,tolerances,init=TRUE,thetas0,
          ...
          thetas.init=ADMM_VH(Xstd, M, K, p,pcore, L, rho,lmbda_grid,maxiter1=50,maxiter2=1000,maxiter3=1000, tol1,tol2,tol3,tolerances,init=FALSE,thetas.zeros,
                          Lsqrt,Lsqrt.inv,t(Lsqrt),temp.inv)
          
          res.thetas=ADMM_VH(Xstd, M, K, p,pcore, L, rho,lmbda_grid,maxiter1,maxiter2,maxiter3, tol1,tol2,tol3,tolerances,init=TRUE,thetas.init,
                   Lsqrt,Lsqrt.inv,t(Lsqrt),temp.inv)
          rm(thetas.init)    
          ind=index[i,j,l]
          Rt=list()
